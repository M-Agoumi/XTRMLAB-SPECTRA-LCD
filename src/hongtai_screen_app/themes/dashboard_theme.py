...
#
# The first 6 built-ins to ship (Neon Horizon, Bubblegum, Panic Mode,
# Mission Control, Midnight Minimal, Arcade RGB) were all drawn on
# procedural backgrounds -- a starfield, a flat grid, a gradient --
# with gauges simply placed on top. Once the photo-backed and
# art-directed presets below existed, those 6 read as obviously
# first-draft next to them, so they were removed at the user's request
# ("can you remove the simple default themes?"). Nothing else depended
# on them: they were pure code, never copied into anyone's config, so
# deleting their entries is the whole removal. Anyone who had saved
# their own version over one of those names keeps it -- that copy lives
# in `presets` and the merge never needed the built-in to exist.
# Northern Lights, from the photo-backed set below, went the same way
# for the same reason.
#
# The 3 remaining originals below were each built to showcase one of
# the bundled "photo" backgrounds (BUNDLED_BACKGROUND_IMAGES) rather
# than a procedural one, and deliberately sparse -- a photo background
# is already visually busy, so piling gauges on it would just fight it
# for attention. City Nights uses a stat-bound text element (`"stat"` +
# `"template"`, see _resolve_text_content()) instead of only
# gauges/bars, doubling as an example layout for that too.
#
...
#
# Circuit Bloom is different from the originals above: it isn't an
# original design, it's a real user's own saved layout ("my preset")
# plus their own uploaded background photo, shipped as a built-in at
# their request so it ships with every install instead of staying
# local to their machine. See its own comment just above its entry
# below.
#
...
# `"template"`, the same mechanism City Nights uses) over a new
# pastel floral photo background, requested as a pair
